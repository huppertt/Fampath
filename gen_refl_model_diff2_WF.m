function [diff_2_model_refl] = gen_refl_model_diff2_WF(parameters,xdata)
...
% basically over here we need to specify that we only want to fit in
% specific regions from wave_start to wave_end but i don't get how to do
% that 
% x = [WF, HHb, HbO2, a, b] 
%xdata = [xdata_original-1:xdata_original+1];
WF = parameters(1);
HHb = parameters(2);
HbO2 = parameters(3);
a = parameters(4);
b = parameters(5);
% here xdata is the wavelength range from 680-850. because the abs/ext
% tables start at 650, we want them indexed from 680-650 etc. 
